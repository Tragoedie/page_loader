...
def download(url: str, directory: str = DEFAULT_PATH) -> str:
    ...
    try:
        ...
        raise ExpectedError('Create folder error: {0}'.format(str(error)))
...
def download_local_files(urls, files_folder) -> None:
    ...
    for url in urls:
        local_file = get_response(url[0]).content
...
def save_data(path_to_data: str, data: Any) -> None:
    """Save web page.

    Args:
        path_to_data: path to save html
    ...
    try:
        with open(path_to_data, 'wb') as data_file:
            log.info('Save to the {0}'.format(path_to_data))
